...
**Crates/Dependencies:** `tokio`, `bytes`, `slab`, `vrl`, `notify`

### Micro-Task 1.1: Zero-Copy Network Listener
*   **Target File:** `prism-core/src/ingest/listener.rs`
*   **Agent Instructions:** Implement an asynchronous `tokio::net::UdpSocket` bound to port 514. Do not allocate new `String` or `Vec<u8>` for every packet. Instead, implement a zero-copy memory pool using the `bytes::BytesMut` or `slab` crate. When a packet arrives, yield a read-only slice (`&[u8]`) and pass it to a high-throughput `flume` channel.
